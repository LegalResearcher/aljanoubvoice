    ...
    <section className="w-full mb-8">
      ...
      <div className="relative group">
        ...
        <div className="flex justify-center -mt-6 sm:-mt-8 relative z-10">
          <div className="bg-white/95 backdrop-blur-sm rounded-full shadow-lg p-2 flex gap-2 overflow-x-auto hide-scroll border border-gray-200">
            {slides.map((_, index) => (
              <button
                key={index}
                onClick={() => goToSlide(index)}
                className={`w-7 h-7 rounded-full text-xs font-bold flex items-center justify-center transition duration-300 flex-shrink-0 ${
                  index === currentIndex
